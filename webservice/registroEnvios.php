	...
	switch($socio){
		...
		case 2://empaque
			...
			if($tipo==2){//Registrar el del empaque
				...
				if($r){
					...
					mysql_query("UPDATE camiones_empaque set disponibilidad_ce = 1 where id_camion = $carro");
				}else
		...
		case 3://distribuidor
			...
			if($tipo==2){//Registrar el envio del distribuidor
				...
				if($r){
					$datos_usuario = "Bien*Envio registrado como pendiente. \n - Proceda a leer todas las cajas y tarimas para completar el envio.";
					mysql_query("UPDATE camiones_distribuidor set disponibilidad_camion_distribuidor = 1 where id_camion_distribuidor = $carro");
				}else
					$datos_usuario ="Error*Error al registrar";
