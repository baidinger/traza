<?php 
	...
	if(!empty($nombreUsuario) && !empty($apellidosUsuario) && !empty($usuario) && !empty($contrasenaUsuario) && !empty($direccionUsuario) && !empty($telefonoUsuario)){
		...
		$consulta = "INSERT INTO usuario_punto_venta(id_punto_venta_fk, id_usuario_fk, nombre_usuario_pv, apellidos_usuario_pv, telefono_usuario_pv, direccion_usuario_pv) VALUES($puntoVentaUsuario, $idUsuarioFK, '$nombreUsuario', '$apellidosUsuario', '$telefonoUsuario', '$direccionUsuario')";
		mysql_query($consulta, $conexion);
