...
?>
 ...
 <?php
	...
	// DATOS DEL ENVÍO
	$consulta = "SELECT fecha_envio, hora_envio FROM envios_empaque WHERE id_envio = $idEnvio";
	$resultado = mysql_query($consulta);
	$row = mysql_fetch_array($resultado);
	...
	$consulta = "SELECT fecha_entrada, hora_entrada FROM entrada_distribuidor WHERE id_envio_fk = $idEnvio";
	$resultado = mysql_query($consulta);
	$row = mysql_fetch_array($resultado);
	...
	class PDF extends FPDF {
		...
		function detallesRecepcion($envio, $descOrden, $descCancelacion, $descRechazo, $tipo){
			...
			$consulta = "SELECT epc_tarima FROM distribuidor_cajas_envio WHERE id_envio_fk = $envio GROUP BY epc_tarima ORDER BY epc_tarima ASC";
			$resultado = mysql_query($consulta);
			while($row = mysql_fetch_array($resultado)){
				...
				$contador = 1;
				$consulta2 = "SELECT epc_caja, enviado_dce, recibido_dce FROM distribuidor_cajas_envio WHERE id_envio_fk = $envio AND epc_tarima = '$epcPallet' ORDER BY epc_caja ASC";
	...
	$pdf->Output($directorio.'recepciondeordendecompradist'.$idOrden.'.pdf', "F");
 ?>