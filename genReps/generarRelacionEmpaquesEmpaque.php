 ...
 <?php
	...
	class PDF extends FPDF {
		...
		function listaUsuarios($idDist){
			...
			$this->Cell(196, 4, 'LISTA DE EMPAQUES', 0, 0, 'L');
			...
			$this->Cell(66, 6, 'NOMBRE DEL EMPAQUE', 1, 0, 'C', 1);
			...
			$consulta = "select id_empaque, nombre_empaque, rfc_empaque, ".
				...
				where id_usuario_que_registro = ".$_SESSION['id_receptor']." ORDER BY nombre_empaque ASC";
			$resultado = mysql_query($consulta);
			while($row = mysql_fetch_array($resultado)){
				$this->SetTextColor(0, 0, 0);

				$this->Cell(10, 5, $contador, 1, 0, 'C', 0);
				$this->Cell(20, 5, str_pad($row['id_empaque'],7,"0",STR_PAD_LEFT), 1, 0, 'C', 0);
				$this->Cell(30, 5, $row['rfc_empaque'], 1, 0, 'L', 0);
				$this->Cell(66, 5, $row['nombre_empaque'], 1, 0, 'L', 0);
				$this->Cell(30, 5, $row['ciudad_empaque'], 1, 0, 'C', 0);
	...
	$pdf->Output($directorio.'empaquesempaque'.$_SESSION['id_empaque'].'.pdf', "F");
 ?>