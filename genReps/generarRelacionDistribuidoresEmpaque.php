 ...
 <?php
	...
	class PDF extends FPDF {
		...
		function listaUsuarios($idDist){
			...
			$this->Cell(196, 4, 'LISTA DE DISTRIBUIDORES', 0, 0, 'L');
			...
			$this->Cell(66, 6, 'NOMBRE DEL DISTRIBUIDOR', 1, 0, 'C', 1);
			...
			$consulta = "select id_distribuidor, nombre_distribuidor, rfc_distribuidor, ".
		...
		where id_usuario_que_registro = ".$_SESSION['id_receptor'] ." ORDER BY nombre_distribuidor ASC";
			$resultado = mysql_query($consulta);
			while($row = mysql_fetch_array($resultado)){
				$this->SetTextColor(0, 0, 0);

				$this->Cell(10, 5, $contador, 1, 0, 'C', 0);
				$this->Cell(20, 5, str_pad($row['id_distribuidor'],7,"0",STR_PAD_LEFT), 1, 0, 'C', 0);
				$this->Cell(30, 5, $row['rfc_distribuidor'], 1, 0, 'L', 0);
				$this->Cell(66, 5, $row['nombre_distribuidor'], 1, 0, 'L', 0);
				...
				$paises = array("MEXICO","ESTADOS UNIDOS","CANADÁ","JAPÓN","AUSTRALIA");
			          	...
			          	switch ($pais_c) {
			          		case '0':
			          			$this->Cell(30, 5, $mexico[$estado_c], 1, 0, 'C', 0);
			          			
			          			break;
			          			case '1':
			          				$this->Cell(30, 5, $eua[$estado_c], 1, 0, 'C', 0);
			          			break;
			          			case '2':
			          				$this->Cell(30, 5, $canada[$estado_c], 1, 0, 'C', 0);
			          			break;
			          			case '3':
			          				$this->Cell(30, 5, $japon[$estado_c], 1, 0, 'C', 0);
			          			break;
			          			case '4':
			          				$this->Cell(30, 5, $australia[$estado_c], 1, 0, 'C', 0);
			          			break;
			          	}
	...
	$pdf->Output($directorio.'distribuidoresempaque'.$_SESSION['id_empaque'].'.pdf', "F");
 ?>