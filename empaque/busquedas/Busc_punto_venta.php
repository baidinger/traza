...
<html>
	...
	<body>
		<?php 
			...
			$placeholder="Nombre punto de venta / RFC / ID";
			$imagen = "pv.png";
	...
	<script type="text/javascript">
		...
		function aplicar(){
			...
			if($("#status").val() == 0 || $("#status").val() == 1) 
				consulta = " AND estado_pv = '" + $("#status").val() + "' " ;
			else consulta = "" ;
			...
			{
				case '0': break;
				case '1': consulta += " AND fecha_registro_pv < '"+$("#fecha_i").val()+"'";  break;
				case '2': consulta += " AND fecha_registro_pv = '"+$("#fecha_i").val()+"'";  break;
				case '3': consulta += " AND fecha_registro_pv > '"+$("#fecha_i").val()+"'";  break;
				case '4': consulta += " AND fecha_registro_pv > '"+$("#fecha_i").val()+"' AND fecha_registro_pv < '"+$("#fecha_f").val()+"'"; break;
			}
			//alert(consulta);
			$("#filtro").val(consulta);
			buscar();
		...
		function lista(){
				...
				$.ajax({
					type: 'POST',
					url: '../genReps/generarRelacionPuntosVentaEmpaque.php',

					success: function(data){
						var urlPDF = "../docs/puntosventaempaque" + <?php print $_SESSION['id_empaque'] ?> + ".pdf";
